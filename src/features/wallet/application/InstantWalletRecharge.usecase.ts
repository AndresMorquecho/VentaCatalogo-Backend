            ...
            const result = await prisma.$transaction(async (tx) => {
                ...
                const incomeFR = await tx.financialRecord.create({
                    data: {
                        ...
                        createdBy,
                        notes: (dto.notes || `Recarga rápida de billetera`) + ` | Cédula: ${(client as any).identificationNumber || '—'} | Orden: — | Pedido: — | Marca: — | Tipo: RECARGA_BILLETERA`,
                        bankAccountId: dto.bankAccountId,
                        source: 'MANUAL',
                        paymentMethod: dto.paymentMethod,
                        movementType: 'INCOME',
                ...
                await tx.financialRecord.create({
                    data: {
                        ...
                        clientDocument: (client as any).identificationNumber ?? null,
                        createdBy,
                        notes: `Ingreso a billetera virtual | Cédula: ${(client as any).identificationNumber || '—'} | Orden: — | Pedido: — | Marca: — | Tipo: RECARGA_BILLETERA`,
                        bankAccountId: dto.bankAccountId,
                        source: 'MANUAL',
                        paymentMethod: dto.paymentMethod,
